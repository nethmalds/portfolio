...
const renderTemplate = (templatePath: string, data: Record<string, any>): string => {
...
const getContactFormHtml = (data: ContactEmailData): string => {
  ...
  return renderTemplate(templatePath, { ...data, submissionTime: new Date().toLocaleString() });
};
...
export const sendAutoReply = async (data: ContactEmailData): Promise<boolean> => {
  try {
    const transporter = createTransporter();
